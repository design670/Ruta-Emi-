...
const CAMPOS_INICIALES: LoginFormData = { usuario: '', contrasena: '', recordarme: false };

export default function LoginForm() {
  const [campos, setCampos] = useState<LoginFormData>(CAMPOS_INICIALES);
  const [errores, setErrores] = useState<LoginFormErrors>({});
  const [enviando, setEnviando] = useState(false);
  const [backendPendiente, setBackendPendiente] = useState(false);

  const actualizar =
    (campo: keyof LoginFormData) =>
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const valor = e.target.type === 'checkbox' ? e.target.checked : e.target.value;
      setCampos((prev) => ({ ...prev, [campo]: valor }));
    };

  const validar = (): LoginFormErrors => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const camposConError = Object.keys(nuevosErrores) as (keyof LoginFormData)[];
