...
import type { RegistroFormData, RegistroFormErrors, SelectOption } from '../types';

const TIPOS_DOCUMENTO: SelectOption[] = [
...
const CAMPOS_INICIALES: RegistroFormData = {
  nombreCompleto: '',
  tipoDocumento: '',
  numeroDocumento: '',
  correo: '',
...
export default function RegistrationForm() {
  const [campos, setCampos] = useState<RegistroFormData>(CAMPOS_INICIALES);
  const [errores, setErrores] = useState<RegistroFormErrors>({});
  ...
  const actualizar =
    ...
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
      ...
      const valor = target instanceof HTMLInputElement && target.type === 'checkbox' ? target.checked : target.value;
      setCampos((prev) => ({ ...prev, [campo]: valor }));
    };

  const validar = (): RegistroFormErrors => {
    const nuevosErrores: RegistroFormErrors = {};
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const camposConError = Object.keys(nuevosErrores) as (keyof RegistroFormData)[];
